...
export async function GET(request: NextRequest) {
  ...
  // 캐시 키 생성
  const cacheKey = `cert_detail_${certNum}`;
  
  // 캐시 확인
  const cachedItem = cache.get(cacheKey);
  if (cachedItem && cachedItem.expiry > Date.now()) {
    console.log(`[캐시] 인증번호 ${certNum}에 대한 캐시된 결과 반환`);
    return NextResponse.json(cachedItem.data);
  }

  try {
    // SafetyKorea API 호출
    const apiUrl = `${BASE_URL}/certificationDetail.json?certNum=${encodeURIComponent(certNum)}`;
    
    console.log(`[API 요청] 인증번호 ${certNum}에 대한 상세 정보 요청`);
    ...
    if (data.resultCode === '2000' || data.resultCode === 2000) {
      console.log(`[캐시] 인증번호 ${certNum}에 대한 결과 캐싱`);
      cache.set(cacheKey, {
        data: data,
