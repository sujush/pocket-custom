...
interface ForwarderDetail {
  ...
  brchAddr: string;           // 지점주소
  hdofAddr: string;           // 본사주소
  bizrNo: string;             // 사업자등록번호
  telNo: string;              // 전화번호
  frwrKoreNm: string;         // 화물운송주선업자한글명
  frwrEnglNm: string;         // 화물운송주선업자영문명
...
const ForwarderFinder: React.FC = () => {
  ...
  const fetchForwarderDetail = async (code: string): Promise<void> => {
    ...
    try {
      ...
      // frwrBrkdRtnVo 태그에서 데이터 추출
      const detail: ForwarderDetail = {
        frwrSgn: getNodeValue(xmlDoc, 'frwrSgn') || code,
        brchAddr: getNodeValue(xmlDoc, 'brchAddr') || '정보 없음',
        hdofAddr: getNodeValue(xmlDoc, 'hdofAddr') || '정보 없음',
        bizrNo: getNodeValue(xmlDoc, 'bizrNo') || '정보 없음',
        telNo: getNodeValue(xmlDoc, 'telNo') || '정보 없음',
        frwrKoreNm: getNodeValue(xmlDoc, 'frwrKoreNm') || '정보 없음',
        frwrEnglNm: getNodeValue(xmlDoc, 'frwrEnglNm') || '정보 없음'
      };
      ...
      if (forwarder) {
        setForwarderDetail({
          frwrSgn: code,
          brchAddr: '정보를 불러올 수 없습니다',
          hdofAddr: '정보를 불러올 수 없습니다',
          bizrNo: '정보를 불러올 수 없습니다',
          telNo: '정보를 불러올 수 없습니다',
          frwrKoreNm: forwarder.nameKr,
          frwrEnglNm: forwarder.nameEn
        });
      }
    } finally {
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <p className="font-semibold">{forwarderDetail.bizrNo}</p>
                        </div>
                      </div>
                      
                      <div>
                        <p className="text-sm text-gray-500">화물운송주선업자명</p>
                        <p className="font-semibold">{forwarderDetail.frwrKoreNm}</p>
                        <p className="text-sm">{forwarderDetail.frwrEnglNm}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{forwarderDetail.brchAddr}</p>
                      </div>
